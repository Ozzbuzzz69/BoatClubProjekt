...
namespace BoatClubLibrary.Bookingdata
{
    public static class BookingRepo
    {
        ...
        /// <returns></returns>
        public static Booking? SearchBooking(string attribute, object Value)
        {
            if (Bookings.Count > 0)
            {
                foreach (KeyValuePair<int, Booking> booking in Bookings)
                {
                    //set the property we are looking for
                    var property = booking.Value.GetType().GetProperty(attribute.ToString());
                    ...
                    */
                    ...
                    foreach (PropertyInfo bookingProperty in booking.Value.GetType().GetProperties())
                    {
                        if (bookingProperty.PropertyType == property.PropertyType)
                        {
                            //return correct value
                            if (bookingProperty.GetValue(booking) == Value) return booking.Value;
                        }
                    }
                }
            }
        ...
        /// <param name="Value"></param>
        /// <returns></returns>
        public static List<Booking> FilterBookings(string attribute, object Value)
        {
                    ...
                    */
                    ...
                    foreach (PropertyInfo bookingProperty in booking.Value.GetType().GetProperties())
                    {
                        if (bookingProperty.PropertyType == property.PropertyType)
                        {
                            ...
                            if (bookingProperty.GetValue(booking) == Value) list.Add(booking.Value);
                        }
                    }
                }
            }
